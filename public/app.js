...
async function init() {
    ...
    const savedPass = localStorage.getItem('gda_admin_pass');
    if (savedPass) {
        state.adminPassword = savedPass;
        const success = await attemptLogin(savedPass);
        if (!success) localStorage.removeItem('gda_admin_pass');
    }
...
function setupEventListeners() {
    ...
    el.adminLoginBtn.addEventListener('click', async () => {
        ...
        const success = await attemptLogin(pass);
        if (success) {
            state.adminPassword = pass;
            localStorage.setItem('gda_admin_pass', pass);
        } else {
